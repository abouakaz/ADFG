...
import adfg.diagram.providers.AdfgMarkerNavigationProvider;
import adfg.diagram.providers.AdfgValidationProvider;
...
public class ValidateAction extends Action {
	...
	public static void runValidation(View view) {
		try {
			if (AdfgDiagramEditorUtil.openDiagram(view.eResource())) {
	...
	public static void runValidation(DiagramEditPart diagramEditPart, View view) {
		...
		AdfgValidationProvider.runWithConstraints(txDomain, new Runnable() {
	...
	private static void validate(DiagramEditPart diagramEditPart, View view) {
		...
		if (target != null) {
			AdfgMarkerNavigationProvider.deleteMarkers(target);
		}
	...
	private static void createMarkers(IFile target, IStatus validationStatus,
		...
		AdfgDiagramEditorUtil.LazyElement2ViewMap element2ViewMap = new AdfgDiagramEditorUtil.LazyElement2ViewMap(
				diagramEditPart.getDiagramView(), collectTargetElements(
						rootStatus, new HashSet<EObject>(), allStatuses));
		for (Iterator it = allStatuses.iterator(); it.hasNext();) {
			IConstraintStatus nextStatus = (IConstraintStatus) it.next();
			View view = AdfgDiagramEditorUtil.findView(diagramEditPart,
	...
	private static void addMarker(EditPartViewer viewer, IFile target,
		...
		AdfgMarkerNavigationProvider.addMarker(target, elementId, location,
	 ...
	 * @generated NOT
	 */
	public static void runValidation(View view, Diagnostic diagnostic) {
		try {
			if (AdfgDiagramEditorUtil.openDiagram(view.eResource())) {
				IEditorPart editorPart = PlatformUI.getWorkbench()
						.getActiveWorkbenchWindow().getActivePage()
				...
				if (editorPart instanceof IDiagramWorkbenchPart) {
					...
					runNonUIValidation(view, diagnostic);
				}
			}
		} catch (Exception e) {
			AdfgDiagramEditorPlugin.getInstance().logError(
	...
	public static void runValidation(DiagramEditPart diagramEditPart,
			View view, final Diagnostic diagnostic) {
		final DiagramEditPart fpart = diagramEditPart;
		final View fview = view;
		TransactionalEditingDomain txDomain = TransactionUtil
				.getEditingDomain(view);
		AdfgValidationProvider.runWithConstraints(txDomain, new Runnable() {
			...
			public void run() {
				validate(fpart, fview, diagnostic);
			}
		});
	}
	 ...
	 * @generated NOT
	 */
	public static void runNonUIValidation(View view, Diagnostic diagnostic) {
		//DiagramEditPart diagramEditPart = OffscreenEditPartFactory.getInstance().createDiagramEditPart(view.getDiagram());
		//runValidation(diagramEditPart, view, diagnostic);
	}
